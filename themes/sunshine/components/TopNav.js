...
const TopNav = props => {
  ...
  const scrollTrigger = throttle(() => {
    ...
    if (header && navTransparent) {
      ...
      nav && nav.classList.replace('dark:bg-sunshine-black-gray', 'transparent')
    } else {
      nav && nav.classList.replace('bg-none', 'bg-white')
      nav && nav.classList.replace('text-white', 'text-black')
      nav && nav.classList.replace('border-transparent', 'border')
      nav && nav.classList.replace('shadow-none', 'shadow-md')
      nav && nav.classList.replace('transparent', 'dark:bg-sunshine-black-gray')
    }
  ...
  return (<div id='top-nav' className='z-40'>
    ...
    {/* 导航栏 */}
    <div id='sticky-nav' className={'top-0 shadow-md fixed bg-none animate__animated animate__fadeIn dark:bg-sunshine-black-gray dark:text-gray-200 text-black w-full z-20 transform duration-200 border-transparent dark:border-transparent'}>
      ...
      <Collapse type='vertical' isOpen={isOpen} className='shadow-xl'>
        <div className='bg-white dark:bg-sunshine-black-gray pt-1 py-2 px-5 lg:hidden '>
          <MenuList {...props}/>
        </div>
      </Collapse>
    </div>
